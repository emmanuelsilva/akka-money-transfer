...
object AccountLedgerActor {
  ...
  //write commands
  final case class OpenAccount(customerId: String, replyTo: ReplyActorType) extends Command with HasReply
  ...
  private def openAccount(cmd: OpenAccount): Effect[Event, Account] = {
    Effect.persist(AccountOpened(cmd.customerId)).thenReply(cmd.replyTo)(_ => StatusReply.Ack)
  }
